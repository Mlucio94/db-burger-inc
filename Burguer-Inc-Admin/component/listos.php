<?php
    ...
    if(isset($_GET['modificar'])){
        ...
        $sql= "update pedido set  estadopedidoid = ( select e.EstadoPedidoID from estadopedido e inner join parmstepestados p on e.EstadoPedidoID = p.EstadoPedidoId where p.id = 4 ) where PedidoID = '$id'";
    ...
    $options = array();
    $sql= "select PedidoID , PedidoNombre , sec_to_time(now() - PedidoFecha) as time, PedidoMontoTotal, case when PedidoNroPago = 0 then 'Cobrar' else 'Pagado' end as pago from pedido p inner join estadopedido e on p.estadopedidoid = e.EstadoPedidoID inner join parmstepestados p2 on e.EstadoPedidoID = p2.EstadoPedidoId where p2.id = 3";
